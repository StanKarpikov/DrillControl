...
bool SparkFun_APDS9960::init()
{
    ...
    //»нициализаци€ I2C в режиме мастер
    i2c_setup();
    i2c_set_addr7(0x39);
...
int SparkFun_APDS9960::readGesture()
{
    ...
    if( !isGestureAvailable() || !(getMode() & B8(01000001) ) ){
        return DIR_NONE;
    }
...
bool SparkFun_APDS9960::readProximity(uint8_t &val)
{
    ...
    if( !wireReadDataByte(APDS9960_STATUS, val) ) {
        return false;
    }   
...
//  I2C_WriteRegister(APDS9960_I2C_ADDR, 1, &val);?
//  set_tout_ms(20); 
//  I2C_WriteRegister(0,1,&val);
 // i2c_7bit_send_onebyte(val,0);
  i2c_7bit_send_onebyte(val,0);
                                        ...
                                        unsigned int len)
{
  ...
  //  }
    //set_tout_ms(20); 
   // I2C_WriteRegister(reg,len,val);
    // I2C_WriteRegister(reg,1,&val);
    
    i2c_7bit_sendR(reg,val,len,0);
...
bool SparkFun_APDS9960::wireReadDataByte(uint8_t reg, uint8_t &val)
{
 ...
 //   while (Wire.available()) {
  ...
  //  set_tout_ms(20);
 //   I2C_ReadRegister(reg,1,&val);
    i2c_7bit_send_onebyte(reg,0);
    i2c_7bit_receive_onebyte(&val,0);
