...
//#define PAUSE(val)  do{TIM1_ARRH = 0; TIM1_ARRL = val; TIM1_CR1 |= TIM1_CR1_CEN; \
//		while(!TIM1_SR1 & TIM1_SR1_UIF); TIM1_SR1 = 0;}while(0)
void delayX(volatile uint32_t count) {
  
for (uint32_t i=0 ; i <count; i+=1000);
}
#define PAUSE(val) delayX(val)
...
void i2c_setup(){
	...
	//TIM1_PSCR = 0; // 16MHz  (for working @ 100kHz: 1us \approx 16tacts)
	//TIM1_CR1 = TIM1_CR1_OPM; // one-pulse mode
}
